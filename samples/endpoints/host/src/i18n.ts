import { FallbackLanguage, LanguageQueryStringKey, SupportedLanguages, type LanguageKey } from "@endpoints/shared";
import type { FireflyRuntime } from "@squide/firefly";
import { getI18nPlugin2, i18nextPlugin2 } from "@squide/i18next";
...
export function createI18nPlugin() {
    const plugin = new i18nextPlugin2(SupportedLanguages, FallbackLanguage, LanguageQueryStringKey);

    // Detection user default language for anonymous pages.
    // If the user is authenticated, the language will be changed for the persisted user
    // preferred user language once the session is loaded.
